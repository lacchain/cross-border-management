...
@SqlResultSetMapping(name = "movementResultMapping", classes = {
    @ConstructorResult(targetClass = MovementResult.class, columns = { @ColumnResult(name = "id", type = String.class),
            @ColumnResult(name = "datetime", type= String.class), @ColumnResult(name = "transfer_type"), @ColumnResult(name = "sender_name"), @ColumnResult(name = "receiver_name"),@ColumnResult(name = "amount", type = Float.class),@ColumnResult(name = "amount_received", type = Float.class), @ColumnResult(name = "sender_currency", type = String.class),@ColumnResult(name = "receiver_currency", type = String.class), @ColumnResult(name = "fee_applied", type = Float.class), @ColumnResult(name = "rate_applied", type = Float.class), @ColumnResult(name = "status"), @ColumnResult(name = "dlt_address") }) })
@NamedNativeQueries({
    @NamedNativeQuery(name = "MovementRepository.getAllMovementsByDltAddress", query = "SELECT DISTINCT movements.id AS id, movements.datetime AS datetime,CASE WHEN UPPER(movements.sender) = UPPER('0X0000000000000000000000000000000000000000') THEN 'TOKENIZATION' ELSE 'ACH' END AS transfer_type,CASE WHEN UPPER(movements.sender) = UPPER('0X0000000000000000000000000000000000000000') THEN 'Citibank' ELSE sender.fullname END AS sender_name,receiver.fullname AS receiver_name,round(movements.amount,2) AS amount, CASE WHEN UPPER(movements.sender) = UPPER('0X0000000000000000000000000000000000000000') THEN round(movements.amount,2) ELSE CASE WHEN movements.status IN (0,1,2,5,6) THEN round((movements.amount/movements.estimatedrate),2) ELSE round((movements.amount/movements.rate),2) END END AS amount_received, CASE WHEN UPPER(movements.sender) = UPPER('0X0000000000000000000000000000000000000000') THEN receiver.currency ELSE sender.currency END AS sender_currency, receiver.currency AS receiver_currency, movements.fee AS fee_applied,CASE WHEN movements.status IN (0,1,2,5,6) THEN round(movements.estimatedrate,4) ELSE round(movements.rate,4) END AS rate_applied, CASE WHEN movements.status = 0 THEN 'REQUESTED' WHEN movements.status = 1 THEN 'APPROVED' WHEN movements.status = 2 THEN 'IN PROGRESS' WHEN movements.status = 3 THEN 'FEE-RATE SETED' WHEN movements.status = 4 THEN 'COMPLETED' WHEN movements.status = 5 THEN 'CANCELLED' ELSE 'FAILED' END AS status,CASE WHEN UPPER(movements.sender) = UPPER(:dltAddress) THEN movements.receiver ELSE movements.sender END AS dlt_address FROM movements, users, accounts, (SELECT users.fullname,accounts.dlt_address, accounts.currency AS currency FROM users INNER JOIN accounts ON accounts.user_id = users.id) AS sender, (SELECT users.fullname, accounts.dlt_address, accounts.currency AS currency FROM users INNER JOIN accounts ON accounts.user_id = users.id) AS receiver	WHERE (UPPER(movements.sender) = UPPER(sender.dlt_address)) and UPPER(movements.receiver) = UPPER(receiver.dlt_address) and (UPPER(movements.sender) = UPPER(:dltAddress) or UPPER(movements.receiver) = UPPER(:dltAddress))", resultSetMapping = "movementResultMapping")})
