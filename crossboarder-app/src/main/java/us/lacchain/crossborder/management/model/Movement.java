...
@NamedNativeQueries({
    @NamedNativeQuery(name = "MovementRepository.getAllMovementsByDltAddress", query = "SELECT DISTINCT movements.id AS id, movements.datetime,CASE WHEN UPPER(movements.sender) = UPPER(:dltAddress) THEN 'TRANSFER OUT' ELSE 'TRANSFER IN' END AS transfer_type,users.company,CASE WHEN movements.sender = '0X0000000000000000000000000000000000000000' THEN movements.amount WHEN movements.sender = UPPER(:dltAddress) THEN movements.amount ELSE (movements.received_amount*movements.rate) END AS amount_received,CASE WHEN movements.sender = '0X0000000000000000000000000000000000000000' THEN accounts.currency WHEN movements.sender = UPPER(:dltAddress) THEN 'USD' ELSE 'DOP' END AS currency,movements.detail,CASE WHEN movements.status = 0 THEN 'REQUESTED' WHEN movements.status = 1 THEN 'APPROVED' WHEN movements.status = 2 THEN 'IN PROGRESS' WHEN movements.status = 3 THEN 'FEE-RATE SETED' WHEN movements.status = 4 THEN 'COMPLETED' ELSE 'FAILED' END AS status,CASE WHEN UPPER(movements.sender) = UPPER(:dltAddress) THEN movements.receiver ELSE movements.sender END AS dlt_address FROM movements INNER JOIN accounts ON UPPER(accounts.dlt_address) = UPPER(dlt_address) INNER JOIN users ON users.id = accounts.user_id WHERE (UPPER(movements.sender) = UPPER(:dltAddress) and UPPER(movements.receiver) = UPPER(dlt_address)) or (UPPER(movements.receiver) = UPPER(:dltAddress) and UPPER(movements.sender) = UPPER(dlt_address))", resultSetMapping = "movementResultMapping")})
